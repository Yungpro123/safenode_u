...
document.addEventListener("DOMContentLoaded", () => {
        ...
        <div class="escrow-card-content">
          <div>
            ...
            <div class="escrow-meta">
              ${symbol === "₦" ? "₦" + " " +Number(amount).toLocaleString() : Number(amount).toLocaleString() +" " + " ₮"}
  ...
  function openEscrowPanel(data) {
    ...
    else if (isSeller) actionBtnHtml = `<button class="btn-primary" id="requestFundsBtn">Request Funds</button>`;
        ...
        <section style="margin-top:18px;">
          ...
          <div style="display:flex;gap:12px;flex-wrap:wrap">
            <div style="background:#fafafa;padding:12px;border-radius:10px;min-width:160px">
              ...
              <div style="font-weight:700">${escapeHtml(data.buyer)}</div>
            </div>
            <div style="background:#fafafa;padding:12px;border-radius:10px;min-width:160px">
              <div style="font-size:.85rem;color:var(--gray)">Seller</div>
              <div style="font-weight:700">${escapeHtml(data.seller)}</div>
            </div>
          </div>
        </section>
...
walletBalanceE.textContent = displayBalance;
      ...
      const sortOrder = ["accepted", "funded", "pending", "completed", "disputed"];
...
const filters = ["All", "Accepted", "Funded", "Completed", "Disputed"];
...
function openEscrowPanel(data) {
  ...
  if (status === "accepted") {
    if (isBuyer) {
      ...
      actionBtnHtml = `
      <button class="btn-primary" id="initiateDisputeBtn"> Request funds</button>
      <button class="btn-secondary" id="initiateDisputeBtn">Initiate Dispute</button>`;
    }
    ...
    actionBtnHtml = `<button disabled class="btn-primary">View Dispute</button>`;
  }
